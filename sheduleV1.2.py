...
def w_time():
	...
	if weekdays[k] != 'Неділя' :
		...
		for i in range(lessons_num + 1):
			...
			e.append(end[i]-time)
		def t(j,a):
			wtime=['До початку %s залишилось %s:%s:%s' % (shedule[k][j+1],s[j+1]//60,s[j+1]%60,60 - now.second),
		...
		for i in range(lessons_num):
			if 0<e[i]<45 and i < lessons_num:
				...
				t(i,1)
			elif e[i]<0 and 0<e[i+1]<25 or (i == 0 and s[0] > 0):
				t(i,0)
			elif e[lessons_num]<0:
...
@bot.message_handler(commands=['t','start',' ','','time'])
def any_msg(m):
	o = 0
...
def dutys(m):
	d,d1,t=duty,duty1,k-2
	d2=d1
	if k==5:
		d2[t]='Сьогодні черговий(а) %s'%d1[5]
	elif k==6:
		d2[t]='Помилка вводу\nСьогодні вихідний'
	elif k<5:
